...
{
    [Header("Pursuit")]
    public float targetRecognitionRange = 8;            // 인식 범위 (이 범위 안에 들어오면 "Pursuit" 상태로 변경
    public float pursuitLimitRange = 10;                // 추적 범위 (이 범위 바깥으로 나가면 "Wander" 상태로 변경)

    [Header("Attack")]
    public GameObject projectilePrefab;                 // 발사체 프리팹
    public Transform projectileSpawnPoint;              // 발사체 생성 위치
    public float attackRange = 5;                       // 공격 범위 (이 범위 안에 들어오면 "Attack" 상태로 변경)
    ...
    public void Setup(Transform target)
    {
        ...
        navMeshAgent = GetComponent<NavMeshAgent>();    // NavMeshAgent 컴포넌트를 가져온다
        this.target = target;                           // 파라미터의 target을 적의 공격 대상으로 설정
