...
public class MemoryPool
{
    ...
    {
        public bool isActive;               // "gameObejct"의 활성화/비활성화 정보
        public GameObject gameObject;       // 화면에 보이는 실제 게임 오브젝트
    }

    private int increaseCount = 5;          // 오브젝트가 부족할 때 Instantiate()로 추사 생성되는 오브젝트 개수
    ...
    /// Instantiate() 메소드를 이용해 오브젝트 생성
    /// 바로 사용하지 않을 수도 있기 때문에
    /// active를 false로 설정하여 보이지 않게 한다.
    ...
    /// Destroy() 메소드를 이용해 오브젝트 삭제
    /// 씬이 바뀌거나 게임이 종료될 때 한번만 수행하여
    /// 모든 게임 오브젝트를 한번에 삭제한다
    ...
    public GameObject ActivatePoolItem()
    {
        ...
        // 현재 생성해서 관리하는 모든 오브젝트 개수와 활성화 상태인 오브젝트 개수 비교
    ...
    /// 현재 활성화 상태인 오브젝트를 removeObject를
    /// 비활성화 상태로 만들어 보이지 않게 한다
    ...
    /// 현재 활성화 상태인 모든 게임오브젝트를 보이지 않게 한다
